 ...
 *
 * @brief            - this func enables the peripheral clk control for usart
 *
 * @param[in]        - base addr of the usart peripheral
 * @param[in]        - enordi macros
 * @param[in]        -
 *
 * @return           - none
 *
 ...
 * @fn               - USART_DeInit
 *
 * @brief            - this func resets various parameters of an USART pin and disbales the peripheral clock
 *
 ...
 * @fn      		  - USART_Init
 *
 * @brief             - this func configs the usart peripheral variables
 *
 ...
 *
 * @brief             - this func sends data using interrupts
 *
 * @param[in]         - usart handle
 * @param[in]         - tx buffer addr
 * @param[in]         - len of data being sent
 *
 * @return            -
 *
 ...
 *
 * @brief             - thsi func receives data
 *
 * @param[in]         - usart handle
 * @param[in]         - receive buffer addr
 ...
 *
 * @brief             - this func sets the baud rate by calculating it
 *
 * @param[in]         - base addr
 * @param[in]         - baud rate
 * @param[in]         -
...
void USART_SetBaudRate(USART_RegDef_t *pUSARTx, uint32_t BaudRate)
{
	...
	//Place the Mantissa part in appropriate bit position, refer USART_BRR (first 4 bits are for fraction part)
	tempreg|=M_part<<4;
 ...
 * @fn      		  - USART_IRQHandler
 *
 * @brief             - this func manages the interrupts received
 *
	...
	if(temp1 && temp2)
	{
		...
		if(pUSARTHandle->TxBusyState == USART_BUSY_IN_TX)
		{
			...
			if(!pUSARTHandle->TxLen)
			{
				...
				pUSARTHandle->pUSARTx->CR1&=~(1<<USART_SR_TC);
...
/*************************Check for IDLE detection flag ********************************************/
	...
	if(temp1 && temp2)
	{
		...
		uint16_t dummyread=pUSARTHandle->pUSARTx->SR;
 ...
 *
 * @brief             - weak func so the user can override and define own function
 *
 * @param[in]         -
 * @param[in]         -
 * @param[in]         -
